...
\card{Duas Tarefas, Nunca Bloqueantes}{
  O programa usa exclusivamente \textbf{\texttt{asyncio}} do MicroPython, em vez de um superlaço (\textit{superloop}) com \texttt{time.sleep()}. As duas tarefas acima rodam como \textbf{corrotinas} sob escalonamento cooperativo, e nenhuma bloqueia a outra: o piscar do LED vermelho nunca atrasa a resposta ao botão, nem a leitura do botão interrompe o LED vermelho.
}
\newpage

% ------------------------------------------------------------------------------
% SLIDE 5: Atualização Dinâmica do OLED
% ------------------------------------------------------------------------------
\slideheader{Atualização Dinâmica}
